...
/* WAL mining (walmine): materialize DDL effects + DML straight from WAL.
 * Base dictionary = relfilenodes of pg_class/pg_attribute (mapped catalogs),
 * supplied via env (mining starts from a known base mapping, like walminer). */
...
main(int argc, char **argv)
{
	...
	/* walmine: base dictionary (mapped-catalog filenodes) from env */
	/* walextract: configure the mining core from env / waldir */
	...
	if (getenv("MINE_PGCLASS") || getenv("MINE_PGATTR"))
		walextract_set_filenodes(wectx,
								 getenv("MINE_PGCLASS") ? (Oid) strtoul(getenv("MINE_PGCLASS"), NULL, 10) : InvalidOid,
								 getenv("MINE_PGATTR") ? (Oid) strtoul(getenv("MINE_PGATTR"), NULL, 10) : InvalidOid);
	if (getenv("MINE_BOOTSTRAP"))
		walextract_set_bootstrap(wectx, atoi(getenv("MINE_BOOTSTRAP")) != 0);
	if (getenv("MINE_PGDATA"))
		walextract_set_pgdata(wectx, getenv("MINE_PGDATA"));
	else if (waldir != NULL)
	{
		size_t		n = strlen(waldir);

		if (n > 7 && strcmp(waldir + n - 7, "/pg_wal") == 0)
		{
			char		pd[1024];

			strlcpy(pd, waldir, sizeof(pd));
			pd[n - 7] = '\0';
			walextract_set_pgdata(wectx, pd);
		}
	}
	...
	for (;;)
	{
		...
		/* walmine: materialize DDL/DML effect from this record */
		walextract_record(wectx, xlogreader_state);
