...
CREATE FUNCTION walextract_wal2batch(start_lsn pg_lsn, end_lsn pg_lsn)
...
REVOKE EXECUTE ON FUNCTION walextract_wal2batch(pg_lsn, pg_lsn) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION walextract_wal2batch(pg_lsn, pg_lsn) TO pg_read_server_files;

-- machine-path single-row summary (counts, payload, peaks)
CREATE FUNCTION walextract_batch_stats(start_lsn pg_lsn, end_lsn pg_lsn)
...
REVOKE EXECUTE ON FUNCTION walextract_batch_stats(pg_lsn, pg_lsn) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION walextract_batch_stats(pg_lsn, pg_lsn) TO pg_read_server_files;
...
CREATE FUNCTION walextract_wal2event_count(start_lsn pg_lsn, end_lsn pg_lsn)
RETURNS TABLE(n_events bigint, event_buf_peak bigint)
AS 'MODULE_PATHNAME', 'walextract_wal2event_count'
LANGUAGE C STRICT PARALLEL UNSAFE;

REVOKE EXECUTE ON FUNCTION walextract_wal2event_count(pg_lsn, pg_lsn) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION walextract_wal2event_count(pg_lsn, pg_lsn) TO pg_read_server_files;
